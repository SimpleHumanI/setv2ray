...
def mac_sub_setall():
	createsublink = (os.path.expanduser(f"{main_path}/subscriptions"))
	if mac_checkExistDir(createsublink) == 0:
		print(f"{color_red}Subscriptions link Directory Already exist")
	else:
		os.mkdir(createsublink)
	suburl = input(f"{color_yellow}Enter Subscriptions URL: {color_reset}")
	print(f"{color_bold}Getting Subscriptios URL ...{color_reset}")

	if mac_checkExistDir(f"{main_path}/subscriptions/*") == 0:
		os.system(f"rm -rf {main_path}/subscriptions/*")
		os.system(f"wget -P {main_path}/subscriptions {suburl} &> /dev/null")
	else:
		os.system(f"wget -P {main_path}/subscriptions {suburl} &> /dev/null")
	...
	decode_sub(createsublink, "subscription.html")
...
def linux_sub_setall():
	...
	if linux_cmd_status(f"ls {main_path}/subscriptions/* &> /dev/null") == 0:
		os.system(f"rm -rf {main_path}/subscriptions/*")
		os.system(f"wget -O {main_path}/subscriptions/subscription.html {linux_suburl} &> /dev/null")
	else:
		os.system(f"wget -O {main_path}/subscriptions/subscription.html {linux_suburl} &> /dev/null")
...
def linux_sub_start():
	...
	if lines:
		...
		if linux_cmd_status(f"ls {main_path}/.pid {go_null}") == 0:
			...
			print(f"{color_green}v2ray Successfully running {color_reset}")
			os.system(f"{linux_v2ray_dir}/v2ray run -c {path_sub}/{selected_sub}")
